...
import { toInnerText } from './renderedTextCollectionSteps.ts'
...
Deno.test('innerText', async (t) => {
	...
	const { renderedTextCollectionSteps } = await import('./renderedTextCollectionSteps.ts')

	const target = document.getElementById('white-space')!
	// const textContent = target.textContent

	await t.step('innerText', () => {
		const result = toInnerText(renderedTextCollectionSteps(target))
		assertEquals(
			result,
			dedent`
				White space
